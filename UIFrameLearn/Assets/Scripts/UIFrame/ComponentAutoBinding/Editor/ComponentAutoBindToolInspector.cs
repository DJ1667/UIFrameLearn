...
public class ComponentAutoBindToolInspector : Editor
{
    ...
    private Dictionary<string, int> _nameDict = new Dictionary<string, int>(); //防止重名
    private Dictionary<int, Transform> _ignoreDict = new Dictionary<int, Transform>(); //忽略列表
    ...
    private UIAutoCreateConfig m_Setting;
    ...
    private void OnEnable()
    {
        ...
        string[] paths = AssetDatabase.FindAssets("t:UIAutoCreateConfig");
        if (paths.Length == 0)
        {
            Debug.LogError("不存在UIAutoCreateConfig");
            return;
        }

        if (paths.Length > 1)
        {
            Debug.LogError("UIAutoCreateConfig数量大于1");
            return;
        }

        string path = AssetDatabase.GUIDToAssetPath(paths[0]);
        m_Setting = AssetDatabase.LoadAssetAtPath<UIAutoCreateConfig>(path);
    ...
    private void AutoBindComponent()
    {
        ...
        var ignoreRootList = childs.Where(t => t.gameObject.name.Contains("Ignore")).ToList();
        foreach (Transform ignoreChild in ignoreRootList.Select(root => root.GetComponentsInChildren<Transform>(true))
            .SelectMany(ignoreChilds => ignoreChilds))
        {
            var hashCode = ignoreChild.GetHashCode();
            if (!_ignoreDict.ContainsKey(hashCode))
                _ignoreDict.Add(hashCode, ignoreChild);
        }
    ...
    private void AddBindData(string name, Component bindCom)
    {
        ...
        if (!_nameDict.ContainsKey(name))
            _nameDict.Add(name, 1);
        else
            _nameDict[name]++;

        name = _nameDict[name] == 1 ? name : (name + _nameDict[name]);
    ...
    private void GenAutoBindCode()
    {
        ...
        using (StreamWriter sw = new StreamWriter($"{codePath}/{className}/{className}.BindComponents.cs"))
        {
